...
/**
 * The yaw the office holds while the colleague reports.
 *
 * `DOOR_YAW * 0.45` in `src/ui/office/Office.tsx`: the head turns 38° toward
 * the doorway as she enters and eases back to 17.1° once she starts speaking.
 * Kept in sync by hand because the office owns the choreography and this file
 * owns the staging; if the two ever disagree, the report-framing assertions
 * below stop describing the shot they claim to describe.
 */
const REPORT_YAW = -((38 * Math.PI) / 180) * 0.45;
...
test.describe('characters (P0.3)', () => {
  ...
  for (const size of REVIEW_SIZES) {
    test(`characters stay clear of the monitor surfaces at ${size.label}`, async ({ page }) => {
      ...
      const characters = {
        colleague: anchorRect(camera, CHARACTER_ANCHORS.colleagueHead, box.width, box.height),
      };
       ...
       * speaks.
       *
       * The centred pose above is the contract's letter — "readable at 1440x900
       * and 1280x720 without overlapping monitor DOM" — but it is not the shot.
       * The office glances to `DOOR_YAW * 0.45` (−17.1°) for the whole of the
       * colleague's report, and a staging that clears the glass head-on can
       * still bury her behind the right-hand panel once the head turns. That is
       * exactly what the first attempt at this pass did, and nothing here
       * caught it: the capture showed an empty patch of wall where she was
       * supposed to be, and every assertion passed.
       */
      const reportCamera = createCamera(box.width, box.height, REPORT_YAW, 0);
      const reportMonitors = MONITORS.map((monitor) =>
        monitorRect(reportCamera, monitor, box.width, box.height),
      );
      const reportHead = anchorRect(
        reportCamera,
        CHARACTER_ANCHORS.colleagueHead,
        box.width,
        box.height,
      );

      expect(reportHead.x0, `report framing: off the left edge ${describe(reportHead)}`).toBeGreaterThan(0);
      expect(reportHead.x1, `report framing: off the right edge ${describe(reportHead)}`).toBeLessThan(box.width);
      expect(reportHead.y0, `report framing: above the frame ${describe(reportHead)}`).toBeGreaterThan(0);
      expect(reportHead.y1, `report framing: below the frame ${describe(reportHead)}`).toBeLessThan(box.height);

      for (const surface of reportMonitors) {
        expect(
          intersects(reportHead, surface),
          `at the report framing the colleague's head ${describe(reportHead)} is behind ` +
            `the ${surface.id} monitor ${describe(surface)} at ${size.label}`,
        ).toBe(false);
      }
